...
namespace MeetingRoom.IntegrationTests
{
    public class SliceFixture
    {
        ...
        public static async Task ExecuteScopeAsync(Func<IServiceProvider, Task> action)
        {
            ...
            {
                var dbContext = scope.ServiceProvider.GetService<ExamContext>();
        ...
        public static Task ExecuteDbContextAsync(Func<ExamContext, Task> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<ExamContext>()));

        public static Task ExecuteDbContextAsync(Func<ExamContext, IMediator, Task> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<ExamContext>(), sp.GetService<IMediator>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<ExamContext, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<ExamContext>()));

        public static Task<T> ExecuteDbContextAsync<T>(Func<ExamContext, IMediator, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<ExamContext>(), sp.GetService<IMediator>()));
        ...
        //public static int NextRoomNumber() => Interlocked.Increment(ref RoomNumber);
    }
}
